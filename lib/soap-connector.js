...
SOAPConnector.prototype.connect = function (cb) {
  ...
  soap.createClient(self.wsdl, self.settings, function (err, client) {
    if (!err) {
      ...
      if (self.settings.security) {
        var sec = null;
        var secConfig = self.settings.security;
        if (debug.enabled) {
...
SOAPConnector.prototype.jsonToXML = function (method, args) {
  ...
  if (input.parts) {
    message = client.wsdl.objectToRpcXML(name, args, alias, ns);
  } else if (typeof (args) === 'string') {
    message = args;
  } else {
    message = client.wsdl.objectToDocumentXML(input.$name, args,
      input.targetNSAlias, input.targetNamespace, input.$type);
  }
  return message;
}

SOAPConnector.prototype.xmlToJSON = function (xml) {
  if(!xml) {
    return {};
  }
  ...
  for (var s in this.client.wsdl.services) {
    ...
    for (var p in service) {
      ...
      for (var m in port) {
        ...
        wsMethod.xmlToJSON = SOAPConnector.prototype.xmlToJSON.bind(self);
